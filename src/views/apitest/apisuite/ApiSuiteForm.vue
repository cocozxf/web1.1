<template>
    <el-form ref="ruleFormRef" :model="apisuite" :rules="rules" label-width="120px" class="demo-ruleForm">
      ...
      <el-form-item label="用例关联">
        <el-table :data="CaseList" style="width: 100%">
          <el-table-column prop="collection_name" label="用例名称" style="width: 85%" show-overflow-tooltip="true" />
    ...
    <el-dialog v-model="infoDialogFormVisible" title="添加用例">
      <el-form-item>
        ...
        <el-table :data="CaseInfoList" style="width: 100%">
          <el-table-column prop="id" label="用例编号" style="width: 10%" />
  ...
  // 2. 加载用例关联信息 
  import { queryByPage, deleteData } from "./Case.js"; // 不同页面不同的接口
  const CaseList = ref([] as any[]); // 关联的用例
  ...
  const searchForm = reactive({ "project_id": '' })// 搜索
